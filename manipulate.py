...
def call_model_sanitize(original_message: str, model: str = MODEL_NAME) -> Tuple[Optional[dict], Optional[str]]:
  """
  调用 OpenAI 模型进行判定与改写。
  ...
  for idx, chat in enumerate(chats):
    try:
      ...
      if not isinstance(rewritten, str):
        errors.append(f"chat[{idx}] rewritten_message 非字符串: {repr(rewritten)}")
        continue
